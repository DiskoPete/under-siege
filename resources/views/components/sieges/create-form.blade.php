...
>
    ...
    <x-ui.field>
        ...
        <div class="space-y-2">
            <input type="url" name="target" required class="text-input"
                   value="{{$siege?->configuration->target ?: null }}" placeholder="{{__('https://...')}}">
    ...
    <x-ui.field>
        ...
        <div x-data="{value: {{$initialDuration}} }" class="flex gap-3">
            <input
                ...
                name="duration"
                min="10"
                max="{{ config('siege.max_duration')}}"
                value="{{$initialDuration}}"
                required
                x-model="value"
            >
            <div class="flex gap-2" x-cloak>
                <div><span x-text="value"></span> {{__('Seconds')}}</div>
            </div>
        </div>
    ...
    <x-ui.field>
        ...
        <div x-data="{value: {{$initialConcurrentUsers}} }" class="flex gap-3">
            <input class="grow" type="range" name="concurrent" min="5" max="{{ config('siege.max_concurrent_users')}}"
                   required x-model="value" value="{{$initialConcurrentUsers}}">
            <div class="flex gap-2" x-cloak>
